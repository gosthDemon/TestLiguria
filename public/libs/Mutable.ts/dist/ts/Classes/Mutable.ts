...
export default class Mutable {
    private mutable: MutableEntity = new MutableEntity();
    ...
    private createLeftSection(uniqueId: string) {
        return `
    ...
    private createRightSection(uniqueId: string) {
            ...
            ${
                !this.mutable.data.every((item) => item.selectable === false) && this.mutable.options.selectableAll == true
                    ...
                    : `
                        ...
                        ${this.createDropdownButton(
                            `${uniqueId}_ItemsPerPage`,
                            `<span id='${uniqueId}_counterPerPage'>10</span>`,
                            `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path fill-rule="evenodd" clip-rule="evenodd" d="M3.54975 8.41967C3.84264 8.12677 4.31752 8.12677 4.61041 8.41967L11.1304 14.9397C11.6075 15.4168 12.3926 15.4168 12.8697 14.9397L19.3897 8.41967C19.6826 8.12677 20.1575 8.12677 20.4504 8.41967C20.7433 8.71256 20.7433 9.18743 20.4504 9.48033L13.9304 16.0003C12.8675 17.0632 11.1326 17.0632 10.0697 16.0003L3.54975 9.48033C3.25685 9.18743 3.25685 8.71256 3.54975 8.41967Z" fill="#292D32"/>
                            </svg>`,
                            true
    ...
    public renderTable(): void {
        if (this.localStorage) {
            const mutableContainer: HTMLElement | null = document.getElementById(this.mutable.container);
            if (mutableContainer == null) {
                console.error("Mutable container element not found in the DOM.");
                return;
            }
            const uniqueId: string = this.localStorage.inMutableHash.id;
            ...
            const container: HTMLElement | null = mutableContainer.querySelector(`#${uniqueId}_table`);
            if (container == null) {
                console.error("Unable to create the table in the DOM.");
            }
            const filteredHeaders: string[] = this.mutable.getHeaders(this.mutable.filteredData[0]);
            const hide: string[] = this.localStorage.hideColumns;
            let data: Record<string, any>[] = [...this.mutable.filteredData];
            const page: number = this.mutable.options.paginator.page;
            ...
            let tableHtml: string = '<table class="data-table"><thead><tr>';
            ...
            filteredHeaders.forEach((header: string) => {
                if (header != "mutable_index") {
                    if (!hide.includes(header) && header != "mutable_index") {
                        let icon: string = "";
                        if (this.mutable?.options?.columns?.length > 0 || this.mutable.options.sortableAll) {
                            ...
                            if ((column != null && column.sortable) || this.mutable.options.sortableAll) {
                                ...
                                if (this.localStorage?.orderBy?.column === header) {
                ...
                if (this.mutable.buttons != null && typeof this.mutable.buttons === "function") {
                    if (this.mutable.buttons(row) != "") {
                        if (this.mutable.buttons != null) {
                            let button: string = this.mutable.buttons(this.mutable.data[row.mutable_index]);
                            if (button != "") {
            ...
            const headers: NodeListOf<HTMLElement> = container.querySelectorAll("th[data-header]");
            headers.forEach((th: HTMLElement) => {
                th.addEventListener("click", () => {
                    this.orderBy(th.getAttribute("data-header") ?? "");
                });
            });
            // Asignar evento change al checkbox
            const selectAllCheckbox = container.querySelector<HTMLInputElement>(`#${uniqueId}_select_all`);
            if (selectAllCheckbox) {
                selectAllCheckbox.addEventListener("change", (event) => this.selectAll(event));
            }
            const recordCheckboxes: NodeListOf<HTMLInputElement> = container.querySelectorAll<HTMLInputElement>("tbody input[type='checkbox'][data-index]");
            recordCheckboxes.forEach((checkbox: HTMLInputElement) => {
                checkbox.addEventListener("change", (event: Event) => {
                    const target = event.target as HTMLInputElement;
                    const rowIndex: number = parseInt(target.getAttribute("data-index") ?? "-1", 10);
    ...
    private renderIfAllColumnsAreHidden() {
    ...
    private renderFooter(): void {
        ...
        if (!(mutableContainer instanceof HTMLElement)) return;
        ...
        const createPageElement = (content: string | number, className: string = "", isDisabled: boolean = false, pageNumber: number | null = null): HTMLDivElement => {
            ...
            if (isDisabled) pageDiv.classList.add("disabled");
    ...
    /*METODOS*/
    private createDropdownButton(id: string, label: string, icon: string, includeOptions = false) {
        return `
        <div class="button-container">
    ...
    // BUILDERS
    private buildHandlerHiddenColumns() {
        if (this.localStorage != null) {
            ...
            if (container != null) {
                ...
                this.mutable.headers.forEach((item) => {
                    let isChecked = this.localStorage?.hideColumns.includes(item) ? "" : "checked";
    ...
    private buildHandlerQuantityPaginator(): void {
        if (this.mutable && this.mutable.validate(this.mutable) && this.localStorage) {
            const uniqueId = this.localStorage.inMutableHash?.id;
            if (!uniqueId) return;

            const optionsRecordsPerPage = this.mutable.options?.paginator?.optionsRecordsPerPage;
            const page = this.mutable.options?.paginator?.page;

            if (!optionsRecordsPerPage || typeof page !== "number") return;
    ...
    private hideColumns() {
        if (this.localStorage) {
            ...
            this.mutable.filteredData.forEach((item) => {
                this.localStorage?.hideColumns.forEach((prop) => {
    ...
    private getRecordsByPage(page: number = 1): Record<string, any>[] {
        if (this.mutable.validate(this.mutable) && this.localStorage) {
            const uniqueId = this.localStorage.inMutableHash?.id;
            const recordsPerPage: number = this.mutable.options.paginator.recordsPerPage;
            let counterPerPage = document.getElementById(`${uniqueId}_counterPerPage`);
            if (counterPerPage) counterPerPage.innerHTML = recordsPerPage.toString();
            const startIndex: number = recordsPerPage * (page - 1);
            const endIndex: number = startIndex + recordsPerPage;
            const currentPageData: Record<string, any>[] = this.mutable.filteredData.slice(startIndex, endIndex);
            return currentPageData;
        }
        return [];
    }
    private search(value: string) {
        let searchableColumns: Column[] = this.mutable.options.columns.filter((filter) => filter.searchable);
        ...
        this.mutable.filteredData = this.mutable.data.filter((record) => {
            return searchableColumns.some((column: any) => {
    ...
    private selectOne(rowIndex: number, isChecked: boolean): void {
        this.mutable.data[rowIndex].selectable = isChecked;
        if (this.localStorage?.inMutableHash.id != undefined) this.createRightSection(this.localStorage?.inMutableHash.id);
    }
    private selectAll(event: Event): void {
        const checkbox = event.target as HTMLInputElement;

        if (checkbox.checked) {
            this.mutable.data.forEach((element: any) => {
    ...
    //HELPERS
    static toggleDropdown(element_id: string): void {
...
(window as any).toggleDropdown = Mutable.toggleDropdown;
